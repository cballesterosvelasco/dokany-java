...
public class MirrorFS extends DokanyFileSystem {
    ...
    public MirrorFS(
        ...
        // TODO: change back to rootPath
        final String path = "c:/development"; //rootPath.toString()
        if (!DokanyUtils.getPath(path).toFile().exists()) {
    ...
    @Override
    public Set<WindowsFindData> findFilesWithPattern(@NonNull final String path, @NonNull final DokanyFileInfo dokanyFileInfo, final String pattern) throws IOException {
        ...
        final HashSet<WindowsFindData> files = new HashSet<>();
    ...
    private void addFindData(@NonNull final Path path, @NonNull final String pattern, @NonNull final Set<WindowsFindData> findData) {
        val normalizedPath = DokanyUtils.normalize(path);
     ...
     * @throws IOException
     */
    private FullFileInfo getInfo(@NonNull final String normalizedPath, final String pathPartToRemove) throws IOException {
        ...
        val attributes = FileAttribute.fromInt(10);
		...
		*/
        ...
        return new FullFileInfo(
                ...
                new WindowsFileTime(DokanyDriver.getRuntime()),
                new WindowsFileTime(DokanyDriver.getRuntime()),
                new WindowsFileTime(DokanyDriver.getRuntime())
    ...
    public FileData read(@NonNull final String path, final int offset, final int readLength) throws IOException {
        ...
        @Cleanup final FileInputStream fis = new FileInputStream(fullPath);
    ...
    public int write(@NonNull final String path, final int offset, @NonNull final byte[] data, final int writeLength) throws IOException {
        ...
        @Cleanup final FileOutputStream fis = new FileOutputStream(fullPath);
     ...
     */
    @Override
    public int getSecurity(@NonNull final String path, final int kind, final byte[] data) throws IOException {
        // TODO Auto-generated method stub
        return 0;
    }
