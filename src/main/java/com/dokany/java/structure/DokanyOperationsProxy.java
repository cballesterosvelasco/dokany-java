 ...
 */
@Slf4j
public final class DokanyOperationsProxy extends DokanyOperations {
    ...
    public DokanyOperationsProxy(@NonNull final DokanyFileSystem fileSystem) {
    ...
    private static void convertCreateFileVariables(
		...
		//DokanyDriver.NativeLibrary.DokanMapKernelToUserCreateFileFlags(rawFileAttributesLong, rawCreateOptionsLong, rawCreateDispositionLong, fileAttributesAndFlags,creationDisposition);
		//LOGGER.trace("rawDesiredAccess: {}", rawDesiredAccess);
	
	
		final long genericDesiredAccess = DokanyDriver.NativeLibrary.DokanMapStandardToGenericAccess(rawDesiredAccess);
	...
	*/
    ...
    private final class ZwCreateFile implements DokanyOperations.ZwCreateFile {
        ...
        public long callback(
                ...
                @NonNull final WindowsSecurityAttributes securityContext,
                final int rawDesiredAccess,
                final int rawFileAttributes,
                final int rawShareAccess,
                final int rawCreateDisposition,
...
*/
    ...
    private final class ReadFile implements DokanyOperations.ReadFile {
        ...
        public long callback(
            ...
            try {
                ...
                if (numRead > 0) {
                    buffer.get().put(offset, data.getBytes(), 0, numRead);
    ...
    private final class WriteFile implements DokanyOperations.WriteFile {
        ...
        public long callback(
            ...
            try {
                ...
                buffer.get().put(offset, data, 0, numberOfBytesToWrite);
                final int written = fileSystem.write(normalizedPath, (int) offset, data, numberOfBytesToWrite);
                numberOfBytesWritten.toNative(DokanyDriver.getRuntime(), buffer.get(), written);
    ...
    private final class GetDiskFreeSpace implements DokanyOperations.GetDiskFreeSpace {
        @Override
        public long callback(
                @NonNull final LongLongByReference rawFreeBytesAvailable,
                @NonNull final LongLongByReference rawTotalNumberOfBytes,
                @NonNull final LongLongByReference rawTotalNumberOfFreeBytes,
                @NonNull final DokanyFileInfo dokanyFileInfo) {
            ...
            //rawFreeBytesAvailable.toNative(DokanyDriver.dokanyNativeLib,    freeSpace.getFreeBytes());
    ...
    private final class GetVolumeInformation implements DokanyOperations.GetVolumeInformation {
        ...
        public long callback(
            ...
            try {
                ...
                //volumeNameBuffer.getPointer(0).putString(0L, DokanyUtils.trimStrToSize(fileSystem.getVolumeInfo().getName(), volumeNameSize));

                rawVolumeSerialNumber.toNative(DokanyDriver.getRuntime(), rawFileSystemNameBuffer.get(), fileSystem.getVolumeInfo().getSerialNumber());

                rawMaximumComponentLength.toNative(DokanyDriver.getRuntime(), rawFileSystemNameBuffer.get(), fileSystem.getVolumeInfo().getMaxComponentLength());

                rawFileSystemFlags.toNative(DokanyDriver.getRuntime(), rawFileSystemNameBuffer.get(), fileSystem.getVolumeInfo().getFileSystemFeatures().toInt());

                // TODO: what to do with this?
                //rawFileSystemNameBuffer.toNative(0L, DokanyUtils.trimStrToSize(fileSystem.getVolumeInfo().getFileSystemName(), rawFileSystemNameSize));
    ...
    private final class Mounted implements DokanyOperations.Mounted {
        @Override
        public long callback(final Pointer dokanyFileInfo) {
            try {
                fileSystem.mounted();
    ...
    private final class GetFileSecurity implements DokanyOperations.GetFileSecurity {
        ...
        public long callback(
            ...
            try {
                ...
                rawSecurityDescriptor.get().put(0, out, 0, rawSecurityDescriptorLength);
                rawSecurityDescriptorLengthNeeded.toNative(DokanyDriver.getRuntime(), rawSecurityDescriptor.get(), expectedLength);
    ...
    private final class SetFileSecurity implements DokanyOperations.SetFileSecurity {
        ...
        public long callback(
            ...
            try {
                ...
                // TODO: getting value but not doing anything???
                rawSecurityDescriptor.get().get(0, data, 0, rawSecurityDescriptorLength);
    ...
    private final class FindStreams implements DokanyOperations.FindStreams {
        ...
        public long callback(
            ...
            val normalizedPath = DokanyUtils.normalize(path.get());
    ...
    static boolean isSkipFile(@NonNull final java.lang.String normalizedPath) {
